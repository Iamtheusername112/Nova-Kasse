...
export function calculateBalance(transactions) {
  ...
  // Pending transfers are EXCLUDED from balance (amount not deducted until admin approves)
  // Only completed transactions affect the balance
  const totalBalance = transactions.reduce((sum, t) => {
    const amount = parseFloat(t.amount || 0);
    // Exclude pending transfers - they don't affect balance until approved
    if (t.type === 'transfer' && t.status === 'pending') {
      return sum; // Skip pending transfers
    }
    // Include all other transactions (completed, failed, etc.)
    return sum + amount;
  }, startingBalance);
